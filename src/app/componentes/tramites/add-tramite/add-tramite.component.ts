...
export class AddTramiteComponent implements OnInit {
  ...
  constructor(private s_tramites: TramitesService, private router: Router,
    private s_clientes: ClientesService, private s_tptramites: TpTramiteService,
    private fb: FormBuilder, private s_facturas: FacturaService,
    private s_liquidatramites: LiquidaTramiteService, 
    private s_rubroxfac: RubroxfacService, private s_rubroadicional: RubroAdicionalService) { }
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      let o_tptramite = document.getElementById("id-tpt" + tptramite.idtptramite) as HTMLElement;
  ...
  listarTpTramites() {
    this.s_tptramites.getListaTpTramite().subscribe(datos => {
  ...
  onSubmit() {
    ...
    this.s_tramites.saveTramite(this.f_tramites.value).subscribe(datos => {
    ...
    setTimeout(() => {
      this.s_facturas.saveFactura(this.f_factura.value).subscribe(datos => {
        this.ultimaf = datos;
        setTimeout(() => {
          this.s_tramites.getListaTramites().subscribe(datos => {
            ultimoTramite = datos[datos.length - 1];
          }, error => console.log(error));
          setTimeout(() => {
            this.f_liquidatramite.value.idfactura_facturas = this.ultimaf;
            this.s_liquidatramites.saveLiquidaTramite(this.f_liquidatramite.value).subscribe(datos => {
  ...
  generarFactura() {
    ...
    this.f_factura.setValue({
      ...
      idcliente: { idcliente: this.f_tramites.value.idcliente_clientes.idcliente },
      nrofactura: null,
  ...
  obtenerRubrosByTpTramite() {
    let s_tptramite = document.getElementById("idtptramite_tptramite") as HTMLSelectElement;
    this.s_rubroadicional.getRubAdiByIdTpTram(1).subscribe(datos => {
    ...
    s_tptramite.addEventListener('change', () => {
      this.rubxfac = {};
      this.v_rubros = [];
      this.f_tramites.value.idtptramite_tptramite = {idtptramite:+s_tptramite.value!};
      this.s_rubroadicional.getRubAdiByIdTpTram(+s_tptramite.value!).subscribe(datos => {
        if (datos.length != 0) {
          this.v_modulo = datos[0].idrubro_rubros.idmodulo_modulos;
          let i = 0;
          datos.forEach(() => {

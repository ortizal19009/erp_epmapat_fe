    ...
    this.rxfService.getByIdfactura(this._facturas[i].idfactura).subscribe({
      next: (datos) => {
        ...
        datos.forEach(() => {
          ...
          let [valoriva, total] = valoresIVA(
            datos[j].idrubro_rubros.swiva,
            baseImponible,
            this.porciva
          );
          r = {
          ...
          let indice = this.vecrubros.findIndex(
          ...
          if (indice == -1) this.vecrubros.push(r);
            ...
            this.vecrubros[indice].valorunitario =
              Math.round(
                (this.vecrubros[indice].valorunitario + r.valorunitario) * 100
              ) / 100;
            this.vecrubros[indice].subtotal =
              Math.round((this.vecrubros[indice].subtotal + r.subtotal) * 100) /
              100;
            this.vecrubros[indice].valoriva =
              Math.round((this.vecrubros[indice].valoriva + r.valoriva) * 100) /
              100;
            this.vecrubros[indice].total =
              Math.round((this.vecrubros[indice].total + r.total) * 100) / 100;
          }
          j++;
        });
        i++;

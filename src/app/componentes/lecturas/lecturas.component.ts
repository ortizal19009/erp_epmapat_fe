...
export class LecturasComponent implements OnInit {
   ...
   imprimir() { this.router.navigate(['/imp-lecturas']); }
   ...
   arecaudar(lectura: any) {
      ...
      this.pli24Service.getBloque(categoria, consumo).subscribe({
         next: resp => {
            if (resp.length == 0) { console.log('No hay Tarifa para esta Categoría y Consumo') }
            else {
               this.tarifa = resp;
               ...
               if (swcate9 || swmunicipio) {
                  ...
                  let rub1001 = Math.round((suma - rub1002 - rub1003 - 0.1) * 100) / 100;
                  ...
                  this.rubros.push(r);
                  r = { descripcion: 'Alcantarillado', valorunitario: num2 + num4 + num7 };
                  this.rubros.push(r);
   ...
   async calcular() {
      for (let i = 0; i < this._lecturas.length; i++) {
         let categoria = this._lecturas[i].idabonado_abonados.idcategoria_categorias.idcategoria;
         let consumo = this._lecturas[i].lecturaactual - this._lecturas[i].lecturaanterior;
   ...
   async getPendientesAbonado(idabonado: number): Promise<number> {
      try {
         const facturasPendientes = await this.facService.getPendientesAbonadoAsync(idabonado);
   ...
   async planillas() {
      ...
      let multa = await this.getPendientesAbonado(this._lecturas[this.kontador].idabonado_abonados.idabonado);
   ...
   sort(columnName: string) {
      console.log('Pasa')
      this._lecturas.sort((a: { [x: string]: number; }, b: { [x: string]: number; }) => {

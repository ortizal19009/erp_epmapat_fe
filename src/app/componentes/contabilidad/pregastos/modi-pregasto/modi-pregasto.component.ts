...
export class ModiPregastoComponent implements OnInit {
   ...
   onSubmit() {
      ...
      this.pregasService.updatePregasto(this.idpresupue, this.formPregasto.value).subscribe({
         next: resp => {
            if (this.antcodpar != codpar) {
               this.ejecuService.actualizarCodpar(this.idpresupue, codpar).subscribe({
                  next: nex => { },
                  error: err => console.error(err.error)
               });
            }

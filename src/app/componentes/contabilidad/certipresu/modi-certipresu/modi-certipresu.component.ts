...
export class ModiCertipresuComponent implements OnInit {
  ...
  listarResponsables(e: any) {
    if (e.target.value != '') {
      this.beneService.findByGrupo(e.target.value, this.gruben).subscribe({
        next: (datos) => {
  ...
  get f() {
    return this.formCertipresu.controls;
  }
  ...
  saveCetiPresu() {
    this.certiService
      .updateCerti(this.idcerti, this.formCertipresu.value)
      .subscribe({
        next: (datos) => {
          this.retornar();
        },
  ...
  valNumero(control: AbstractControl) {
    return this.certiService
      .getNumero(control.value)
      .pipe(
        map((result) =>
          result != null && control.value != this.antnumero
            ? { existe: true }
            : null
        )
      );

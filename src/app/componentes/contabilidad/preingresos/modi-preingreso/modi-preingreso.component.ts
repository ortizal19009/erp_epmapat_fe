...
export class ModiPreingresoComponent implements OnInit {
  ...
  constructor(
    ...
    public preingService: PreingresoService,
  ...
  get codpar() {
    return this.preingForm.get('codpar');
  }

  onSubmit() {
    this.preingService
      .updatePreingreso(this.idpresupue, this.preingForm.value)
      .subscribe({
  ...
  regresar() {
    this.router.navigate(['/info-preingreso']);
  }
  ...
  buscarClasificador() {
    console.log(
      'Envia: ' +
        this.formBusClasificador.value.codpar +
        '  ' +
        this.formBusClasificador.value.nompar
    );
    this.clasiService
      .getParingreso(
        this.formBusClasificador.value.codpar,
      ...
      .subscribe({
        next: (datos) => {
          ...
          console.log(
            'this._clasificador.length= ' + this._clasificador.length
          );
        },
        error: (err) => console.log(err.error),
      });
  ...
  valCodpar(control: AbstractControl) {
    return this.preingService
      .getByCodigoI(control.value)
      .pipe(
        map((result) =>

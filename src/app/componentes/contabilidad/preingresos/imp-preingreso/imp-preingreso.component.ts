...
export class ImpPreingresoComponent implements OnInit {
   ...
   constructor(public fb: FormBuilder, private router: Router, private preingService: PreingresoService,
      private ejeService: EjecucionService, private clasiService: ClasificadorService) { }
   ...
   saldos() {
      let i = 0;
      this._presupuei.forEach((objEjecu: { reforma: number, devengado: number, cobrado: number }) => {
         this.ejeService.getTotalModi(this._presupuei[i].codpar, this.formImprimir.value.desde, this.formImprimir.value.hasta).subscribe({
            next: resp => objEjecu.reforma = resp,
            error: err => console.error('Error al obtener el total de reformas:', err.error)
         });
         this.ejeService.getTotalDeven(this._presupuei[i].codpar, this.formImprimir.value.desde, this.formImprimir.value.hasta).subscribe({
            next: resp => objEjecu.devengado = resp,
            error: err => console.error('Error al obtener el total devengado:', err.error)
         });
         this.ejeService.getTotalCobpagado(this._presupuei[i].codpar, this.formImprimir.value.desde, this.formImprimir.value.hasta).subscribe({
            next: resp => objEjecu.cobrado = resp,
            error: err => console.error('Error al obtener el total cobrado:', err.error)
         });
         i++
      });
   }
      ...
      this.preingService.getPartidas().subscribe({
         next: resp => {
            ...
            for (let presupue of this._presupuea) {
               ...
               this.ejeService.getCodparFecha(this.cuenta, this.formImprimir.value.desde, this.formImprimir.value.hasta).subscribe({
                  next: (datos) => {
                     ...
                     this.arreglo2.push({
                        codpar: presupue.codpar, nompar: presupue.nompar, inicia: presupue.inicia, refo: presupue.refo, codi: presupue.codi,
                        deve: presupue.deve, cob: presupue.cob, saldo_dev: presupue.saldo_dev, saldo_cob: presupue.saldo_cob
                     });
   ...
   async buscar_clasificador(presupue: any) {
      this.clasiService.getByCodpar(this.nivel_par).subscribe({
         next: resp => {
            ...
            const objetoEncontrado = this.arreglo2.find(x1 => x1.codpar === this.cta_cla);
            if (objetoEncontrado === undefined) {
               this.arreglo2.push({
                  codpar: this.cta_cla, nompar: this.nom_cla, inicia: this.ced_inicia, refo: this.ced_refo, codi: this.ced_codi,
                  deve: this.ced_deve, cob: this.ced_cob, saldo_dev: this.ced_saldo_dev, saldo_cob: this.ced_saldo_cob, negrita: 1
               });
               this.ced_inicia = 0;
               this.ced_refo = 0;
               this.ced_codi = 0;
   ...
   imprimeSaldos() {
      ...
      doc.setFont("times", "normal"); doc.setFontSize(12); doc.text("SALDOS DE LAS PARTIDAS DE INGRESO", m_izquierda, 16)
      doc.setFont("times", "normal"); doc.setFontSize(12); doc.text("FECHA: " + this.formImprimir.value.hasta, m_izquierda, 22)
      let cabecera = ['PARTIDA', 'NOMBRE', 'INICIAL', 'REFORMA', 'CODIFICADO', 'DEVENGADO', 'COBRADO', 'SAL.DEVENGA', 'SAL.COBRO'];
      ...
      this._presupuei.forEach(() => {
         ...
         totales[2] = totales[2] + this._presupuei[i].inicia + this._presupuei[i].reforma;
         totales[3] = totales[3] + this._presupuei[i].devengado;
         totales[4] = totales[4] + this._presupuei[i].cobrado;
         datos.push([this._presupuei[i].codpar, this._presupuei[i].nompar, this._presupuei[i].inicia,
         this._presupuei[i].reforma, this._presupuei[i].inicia + this._presupuei[i].reforma,
         this._presupuei[i].devengado, this._presupuei[i].cobrado]);
         i++;
      });
      datos.push(['', 'TOTAL: ' + this._presupuei.length, totales[0], totales[1], totales[2], totales[3], totales[4]]);
   ...
   imprimeCedula() {
      ...
      let m_izquierda = 10;
      doc.setFont("times", "bold"); doc.setFontSize(16); doc.text("EpmapaT", m_izquierda, 10);
      doc.setFont("times", "normal"); doc.setFontSize(12); doc.text("CÉDULA PRESUPUESTARIA DE INGRESOS", m_izquierda, 16)
      doc.setFont("times", "normal"); doc.setFontSize(12); doc.text("DEL " + this.formImprimir.value.desde + " AL " + this.formImprimir.value.hasta, m_izquierda, 22)
      let cabecera = ['PARTIDA', 'DENOMINACIÓN', 'INICIAL', 'REFORMA', 'CODIFICADO', 'DEVENGADO', 'COBRADO', 'SAL.DEVENGA', 'SAL.COBRO'];

      // Datos para el body de la cedula
      this.array = [];
      this._presupuea = this.arreglo2;
      this._presupuea.forEach((presupuea: any) => {
         if ((presupuea.inicia != 0 || presupuea.refo != 0 || presupuea.deve != 0 || presupuea.cob != 0)) {
            this.array.push([presupuea.codpar, presupuea.nompar, presupuea.inicia, presupuea.refo,
            presupuea.codi, presupuea.deve, presupuea.cob, presupuea.saldo_dev, presupuea.saldo_cob]);
         };
      });
      this.array.sort();
      ...
      autoTable(doc, {
         ...
         didParseCell: function (hookData) {
            ...
            if (hookData.column.index === 0 && hookData.cell.raw!.toString().length <= 8) {
   ...
   exportaCedula() {
      ...
      //Fila 3 Cabecera 
      const cabecera = worksheet.addRow(['Partida', 'Denominación', 'Inicial', 'Reforma', 'Codificado', 'Devengado', 'Cobrado', 'Sal.Devenga', 'Sal.Cobro']);
      cabecera.eachCell(celda => {
         celda.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '002060' } };
         celda.font = { bold: true, name: 'Times New Roman', color: { argb: 'FFFFFF' } };
      });
      ...
      this._presupuea.forEach((presupuea: any) => {
         if ((presupuea.inicia != 0 || presupuea.refo != 0 || presupuea.deve != 0 || presupuea.cob != 0)) {
            datos.push([presupuea.codpar, presupuea.nompar, presupuea.inicia, presupuea.refo,
            presupuea.codi, presupuea.deve, presupuea.cob, presupuea.saldo_dev, presupuea.saldo_cob]);
         };
      });
      ...
      datos.forEach((item: any) => {
         worksheet.addRow([item[0], item[1], item[2], item[3], item[4], item[5], item[6], item[7], item[8]]);
      });
      ...
      anchoConfig.forEach(config => { worksheet.getColumn(config.columnIndex).width = config.widthInChars; });

...
export class AddCuentaComponent implements OnInit {
   ...
   constructor(private fb: FormBuilder, private cuentasService: CuentasService,
      private nivelesService: NivelesService, private router: Router) {
   ...
   listaNiveles() {
      this.nivelesService.getListaNiveles()
         .pipe(
            switchMap(resp => {
               this._niveles = resp;
         ...
         .subscribe(() => {
            this.crearForm();
            this.formCuenta.controls['idnivel'].setValue(this._niveles[this.idnivel - 1]);
         });
   }
   ...
   crearForm() {
      ...
      this.formCuenta = this.fb.group({
         codcue: [this.grucue + '.', [Validators.required, Validators.minLength(this.nivcue), Validators.maxLength(this.nivcue)], this.valGrupo.bind(this)],
   ...
   valNomcue(control: AbstractControl) {
      return this.cuentasService.getByNomcue(control.value)
         .pipe(

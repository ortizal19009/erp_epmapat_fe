...
export class AddEjecucionComponent implements OnInit {
  ...
  constructor(
    ...
    public ejecuService: EjecucionService,
    private preingService: PreingresoService,
    private parent: EjecucionComponent,
  ...
  onSubmit() {
    this.partida.idpresupue = this._partidas[0].idpresupue;
    this.formEjecucion.value.idpresupue = this.partida;
    ...
    this.ejecuService.saveEjecucion(this.formEjecucion.value).subscribe({
      ...
      error: (err) =>
        console.error('Al guardar la Partida en la Reforma: ', err.error),
    });
  }
  ...
  listarxCodigoNombre(e: any) {
    if (e.target.value != '') {
      if (this.reforma.tipo == 'I') {
        this.preingService
          .getByCodigoNombre(e.target.value.toLowerCase())
          .subscribe({
            next: (datos) => {
              this._partidas = datos;
              const inputValue = e.target.value;
              const selectedPartida = this._partidas.find(
                (partida: { codpar: any }) => partida.codpar === inputValue
              );
              if (selectedPartida) {

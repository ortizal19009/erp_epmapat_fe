...
export class AddBenextranComponent implements OnInit {
  ...
  constructor(
    ...
    private benextranService: BenextranService,
  ...
  f1(fila: number) {
    ...
    return control.controls.find(
      (c: { get: (arg0: string) => any }) => c.get('id')!.value === fila
    );
  }
  ...
  onBeneSelected(e: any, i: number) {
    ...
    grupo
      .get('idbene')
      ?.setValidators([Validators.required, this.valBenefi.bind(this)]);
  ...
  //          iddocumento: [this.iAsiento.iddocumento, Validators.required],

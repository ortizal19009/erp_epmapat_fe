...
export class ListarAbonadosComponent implements OnInit {
   ...
   constructor(public fb: FormBuilder, private aboService: AbonadosService, private router: Router,
   ...
   ngOnInit(): void {
      ...
      this.buscarAbonadoForm.get('selecTipoBusqueda')?.valueChanges.subscribe((valor) => {
   ...
   public listarAbonados() {
      this.aboService.getListaAbonados().subscribe({
         next: datos => {
   ...
   eliminarAbonado(idabonado: number) {
      localStorage.setItem("idabonadoToDelete", idabonado.toString());
   }

   aprobarEliminarAbonado() {
      let ida = localStorage.getItem("idabonadoToDelete");
      this.aboService.deleteAbonado(+ida!).subscribe(datos => {
         localStorage.setItem("mensajeSuccess", "Abonado eliminado");
         this.listarAbonados();
      }, error => console.log(error));
      localStorage.removeItem("idabonadoToDelete");
   }

   modificarAbonado(abonado: Abonados) {
      let i_buscarAbonado = document.getElementById("buscarAbonado") as HTMLInputElement;
   ...
   onSubmit() {
      ...
      if (+this.buscarAbonadoForm.value.selecTipoBusqueda == 1 && this.buscarAbonadoForm.value.buscarAbonado != '') {
         this.aboService.getListaByidabonado(+this.buscarAbonadoForm.value.buscarAbonado!).subscribe({
            next: datos => this._abonados = datos,
            error: err => console.log(err.error)
         });
      } else if (+this.buscarAbonadoForm.value.selecTipoBusqueda == 2 && this.buscarAbonadoForm.value.buscarAbonado != '') {
         this.aboService.getListaByNombreCliente(this.buscarAbonadoForm.value.buscarAbonado).subscribe(datos => {
            this._abonados = datos;
         }, error => console.log(error));
      } else if (+this.buscarAbonadoForm.value.selecTipoBusqueda == 3 && this.buscarAbonadoForm.value.buscarAbonado != '') {
         this.aboService.getListaByidentIficacionCliente(this.buscarAbonadoForm.value.buscarAbonado).subscribe(datos => {
            this._abonados = datos;
         }, error => console.log(error));
      }
   ...
   pdf() {
      ...
      doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("ABONADOS", m_izquierda, 16);

      var datos: any = [];
      var i = 0;
      this._abonados.forEach(() => {
         datos.push([this._abonados[i].idcliente_clientes.nombre, this._abonados[i].idcliente_clientes.cedula,
         this._abonados[i].idabonado, this._abonados[i].idcategoria_categorias.descripcion,
         this._abonados[i].idruta_rutas.descripcion, this._abonados[i].direccionubicacion]);
         i++;
      });
      ...
      autoTable(doc, {
         head: [['Nombre', 'Identificación', 'Cuenta', 'Categoría', 'Ruta', 'Dirección']],
         theme: 'grid',
      ...
      this.aboService.getCampos().subscribe({
         next: datos => {
            ...
            this._campos.forEach((abonado: any) => {
               const row = [abonado.idabonado, abonado.nombre, abonado.cedula, abonado.direccion,
               abonado.direccionubicacion, abonado.telefono, abonado.fechanacimiento, abonado.email];
   ...
   exporta1() {
      ...
      const cabecera = ['Nombre', 'Identificación', 'Cuenta', 'Categoría', 'Ruta', 'Dirección'];
      ...
      this._abonados.forEach((abonado: any) => {
         const row = [abonado.idcliente_clientes.nombre, abonado.idcliente_clientes.cedula, abonado.idabonado,
         abonado.idcategoria_categorias.descripcion, abonado.idruta_rutas.descripcion, abonado.direccionubicacion];
         worksheet.addRow(row);
      });

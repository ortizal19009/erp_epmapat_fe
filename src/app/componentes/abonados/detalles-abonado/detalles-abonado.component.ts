...
export class DetallesAbonadoComponent implements OnInit {
   ...
   constructor(private aboService: AbonadosService, private facService: FacturaService,
      private rubxfacService: RubroxfacService, private lecService: LecturasService,
   ...
   obtenerDatosAbonado() {
      ...
      this.aboService.getByIdabonado(+idabonado!).subscribe({
         next: datos => {
            ...
            this.abonado.categoria = this._abonado[0].idcategoria_categorias.descripcion;
   ...
   grafico(idabonado: number): void {
      ...
      for (let i = 0; i <= this._lecturas.length - 1; i++) {
         emision = this._lecturas[i].idrutaxemision_rutasxemision.idemision_emisiones.emision;
         emision = '20' + emision.substring(0, 2) + '-' + emision.substring(2, 4)
         y.push({
            mes: emision,
            consumo: this._lecturas[i].lecturaactual - this._lecturas[i].lecturaanterior
         })
      }
   ...
   cerrarGrafico() { this.grafic = false; }
}

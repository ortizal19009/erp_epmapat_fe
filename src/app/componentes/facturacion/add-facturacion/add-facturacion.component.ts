...
export class AddFacturacionComponent implements OnInit {
   ...
   constructor(private router: Router, private fb: FormBuilder, private clieService: ClientesService,
      public fb1: FormBuilder, public fb2: FormBuilder, private moduService: ModulosService, private usoiService: UsoitemsService,
      private catiService: CatalogoitemService, private factuService: FacturacionService, private facService: FacturaService,
      private ixfService: ItemxfactService, private rubxfacService: RubroxfacService, private rubService: RubrosService,
   ...
   ngOnInit(): void {
      ...
      this.formCuotas = this.fb2.group({
         cliente: '', total: '', formapago: 1, cuotas: 1
      });
      ...
      seccion.addEventListener("change", () => {
         ...
         this.usoiService.getByIdmodulo(this.formDetalle.value.seccion.idmodulo).subscribe({
      ...
      uso.addEventListener("change", () => {
         ...
         this.catiService.getByIdusoitems(this.formDetalle.value.uso.idusoitems).subscribe({
            next: datos => {
   ...
   onSubmit() {
      ...
      if (this.totfac + this.totiva > 0) this.formCuotas.controls['total'].setValue((this.totfac + this.totiva).toFixed(2).toString());
   }
   ...
   itemxfacturacion() {
      ...
      this.arrRubros.forEach(() => {
         ...
         this.catiService.getById(this.arrRubros[i][4]).subscribe({   //El elemento cuatro es el Id del Producto
            next: resp => {
   ...
   rubrosxfactura() {
      ...
      this.arrRubros.forEach(() => {
         ...
         this.rubService.getRubroById(this.arrRubros[i][5]).subscribe({   //El elemento 5 es el Id del Rubro
            next: resp => {
   ...
   listarFacturacion() { this.router.navigate(['facturacion']); }
   ...
   aniadir(producto: Catalogoitems) {
      let nuevo = this.arrRubros.push([producto.idrubro_rubros.descripcion.toString(), 1,
      +producto.idrubro_rubros.valor!, +producto.idrubro_rubros.swiva!, +producto.idcatalogoitems!, +producto.idrubro_rubros.idrubro!]);
      this.subtotal();
   }
   ...
   todos() {
      ...
      this._productos.forEach(() => {
         this.arrRubros.push([this._productos[i].idrubro_rubros.descripcion.toString(), 1,
         +this._productos[i].idrubro_rubros.valor!, +this._productos[i].idrubro_rubros.swiva!, +this._productos[i].idcatalogoitems!,

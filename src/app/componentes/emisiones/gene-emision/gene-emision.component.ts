...
export class GeneEmisionComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.idemision === 0) {
      this.emiServicio.ultimo().subscribe({
        next: (item: any) => {
          this.idemision = item.idemision;
          this.setEmisionesData(this.idemision);
        },
      });
    } else {
      this.setEmisionesData(this.idemision);
  ...
  setEmisionesData(idemision: any) {
    this.emiServicio.getByIdemision(idemision).subscribe({
      next: (resp) => {
  ...
  async generaRutaxemision() {
    ...
    for (let i = 0; i < this.totalrutas; i++) {
      ...
      try {
        const nuevaRutaxemision =
          await this.ruxemiService.saveRutaxemisionAsync(rutasxemision);
  ...
  async generaLecturas(i: number, nuevarutaxemi: any) {
    try {
      const abonados = await this.aboService.getByIdrutaAsync(
        this._rutas[i].idruta
      );
            ...
            next: async (item: any) => {
              ...
              if (item.length === 0) {
                ...
                try {
                  ...
                  nuevoIdfactura = await this.facService.saveFacturaAsyncId(
                    planilla
                  );
                } catch (error) {
                ...
                try {
                  let lecturaanterior =
                    await this.lecService.getUltimaLecturaAsync(
                ...
                lectura.idcategoria =
                  abonados[k].idcategoria_categorias.idcategoria;

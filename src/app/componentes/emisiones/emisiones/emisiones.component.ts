...
export class EmisionesComponent implements OnInit {
   ...
   buscar() {
      this.emiService.getDesdeHasta(this.formBuscar.value.desde, this.formBuscar.value.hasta).subscribe({
         next: datos => {
   ...
   modificar(idemision: number) { this.router.navigate(['modiemision', idemision]); }
   ...
   pdf() {
      ...
      doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("EMISIONES", m_izquierda, 16);
      ...
      this._emisiones.forEach(() => {
         datos.push([nombreEmision.transform(this._emisiones[i].emision), this._emisiones[i].m3, this._emisiones[i].fechacierre]);
         i++;
      });
   ...
   pdf1() {
      ...
      var doc = new jsPDF();
      doc.setFont("times", "bold"); doc.setFontSize(16); doc.text("EpmapaT", m_izquierda, 10);
      doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("RUTAS POR EMISIÓN", m_izquierda, 16);
      doc.setFont("times", "bold"); doc.setFontSize(11); doc.text('Emisión: ', m_izquierda, 20);
      doc.setFont("times", "normal"); doc.setFontSize(11); doc.text(nombreEmision.transform(this.selEmision), m_izquierda + 16, 20);
      ...
      this._rutasxemi.forEach(() => {
         ...
         datos.push([i + 1, this._rutasxemi[i].idruta_rutas.codigo, this._rutasxemi[i].idruta_rutas.descripcion,
            fecha, this._rutasxemi[i].m3]);
         i++;
      });
      ...
      autoTable(doc, {
         ...
         didParseCell: function (data) {
            ...
            var columna = data.column.index;
            if (columna > 0 && typeof data.cell.raw === 'number') { data.cell.text = [data.cell.raw.toLocaleString('en-US')]; }
            if (fila === datos.length - 1 || columna == 0) { data.cell.styles.fontStyle = 'bold'; } // Total Bold
         }
      });
      addPageNumbers();
   ...
   async exporta0() {
      ...
      this._emisiones.forEach((item: any) => {
         const row = [nombreEmision.transform(item.emision), item.m3, item.fechacierre];
         worksheet.addRow(row);
      });

      //Coloca la fila del Total
      worksheet.addRow(['TOTAL']);
      worksheet.getCell('A' + (this._emisiones.length + 4).toString()).font = { bold: true }
      let celdaB = worksheet.getCell('B' + (this._emisiones.length + 4).toString());
      celdaB.numFmt = '#,##0';
      celdaB.font = { bold: true }
      celdaB.value = { formula: 'SUM(B4:' + 'B' + (this._emisiones.length + 3).toString() + ')', result: 0, sharedFormula: undefined, date1904: false };
   ...
   async exporta1() {
      ...
      worksheet.addRow(['', '', 'Rutas por Emisión: ' + nombreEmision.transform(this.selEmision)]);
      ...
      this._rutasxemi.forEach((item: any) => {
         ...
         if (item.fechacierre != null) {
            ...
            celdaDi.value = { formula: fecha, result: 0, sharedFormula: undefined, date1904: false };
      ...
      //Coloca la fila del Total
      worksheet.addRow(['', '', 'TOTAL']);
      worksheet.getCell('C' + (this._rutasxemi.length + 4).toString()).font = { bold: true }
      let celdaE = worksheet.getCell('E' + (this._rutasxemi.length + 4).toString());
      celdaE.numFmt = '#,##0';
      celdaE.font = { bold: true }
      celdaE.value = { formula: 'SUM(E4:' + 'E' + (this._rutasxemi.length + 3).toString() + ')', result: 0, sharedFormula: undefined, date1904: false };

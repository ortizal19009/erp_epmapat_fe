...
export class AddClienteComponent implements OnInit {
   ...
   constructor(public fb: FormBuilder, private cliService: ClientesService,
      public nacService: NacionalidadService, private router: Router,
      public pjService: PersoneriaJuridicaService, public tpidentiService: TpidentificaService) { }
   ...
   ngOnInit(): void {
      ...
      this.formCliente = this.fb.group({
         ...
         cedula: ['', Validators.required, [this.valIdentifica.bind(this), this.busIdentifica.bind(this)]],
      ...
      const idnacionalidadControl = this.formCliente.get('idnacionalidad_nacionalidad')!;
      const idtpidentificaControl = this.formCliente.get('idtpidentifica_tpidentifica')!;
   ...
   add_nacionalidad() { this.router.navigate(['/nacionalidades']); }
   ...
   retornarListaClientes() { this.router.navigate(['/clientes']); }

   onSubmit() {
      this.nacionalidad.idnacionalidad = this.formCliente.get('idnacionalidad_nacionalidad')!.value;
      this.formCliente.value.idnacionalidad_nacionalidad = this.nacionalidad;
      this.tpidentifica.idtpidentifica = this.formCliente.get('idtpidentifica_tpidentifica')!.value;
      this.formCliente.value.idtpidentifica_tpidentifica = this.tpidentifica;
      this.pjuridica.idpjuridica = this.formCliente.get('idpjuridica_personeriajuridica')!.value;
   ...
   valNombre(control: AbstractControl) {
      return this.cliService.valNombre(control.value.toLowerCase())
         .pipe(
   ...
   valIdentifica(control: AbstractControl) {
      ...
      switch ( this.tpidenti ) {
         ...
         default: return of({ invalid: true });
      }
   ...
   busIdentifica(control: AbstractControl) {
      return this.cliService.valIdentificacion(control.value)
         .pipe(
            map(result => result ? { existe: true } : null)
         );
   }

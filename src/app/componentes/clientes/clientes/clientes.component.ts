...
export class ClientesComponent implements OnInit {
   ...
   constructor(public fb: FormBuilder, private clieService: ClientesService, private router: Router,
      private coloresService: ColoresService, private aboService: AbonadosService, public authService: AutorizaService) { }
   ...
   ngOnInit(): void {
      ...
      this.formBuscar = this.fb.group({
         nombreIdentifi: [buscaClientes, [Validators.required, Validators.minLength(4)]],
         filtro: '',
      });

      let nombreIdentifi = document.getElementById("nombreIdentifi") as HTMLInputElement;
   ...
   onSubmit() {
      if (this.formBuscar.value.nombreIdentifi == '' || this.formBuscar.value.nombreIdentifi == null) {
         this._clientes = [];
      } else {
         this.clieService.getByNombreIdentifi(this.formBuscar.value.nombreIdentifi).subscribe({
            next: datos => {
               sessionStorage.setItem('buscaClientes', this.formBuscar.controls['nombreIdentifi'].value.toString());
   ...
   eliminarCliente(cliente: Clientes) {
      ...
      this.aboService.tieneAbonados(cliente.idcliente).subscribe({
         ...
         error: err => console.error('Al buscar los Abonados del Cliente: ', err.error),
      });
   }
   ...
   irAddClientes() { this.router.navigate(["add-cliente"]); }
   ...
   pdf() {
      ...
      doc.setFont("times", "normal"); doc.setFontSize(12); doc.text("LISTA DE CLIENTES", m_izquierda, 16)

      var datos: any = [];
      let calc: string; let swiva: String;
      var i = 0;
      this._clientes.forEach(() => {
         datos.push([i + 1, this._clientes[i].nombre, this._clientes[i].cedula, this._clientes[i].direccion, this._clientes[i].email]);
         i++;
      });
      ...
      autoTable(doc, {
         theme: 'grid',
         headStyles: { fillColor: [109, 110, 15], fontStyle: 'bold', halign: 'center' },

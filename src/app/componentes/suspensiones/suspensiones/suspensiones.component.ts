...
export class SuspensionesComponent implements OnInit {
   ...
   constructor(private router: Router, private suspeService: SuspensionesService, private coloresService: ColoresService,
   ...
   nuevaSuspension() { this.router.navigate(['add-suspension']); }

   info(suspension: any) {
      sessionStorage.setItem('idsuspensionToInfo', suspension.idsuspension.toString());
      this.router.navigate(['info-suspension'])
   }

   buscarxFecha() {
      if (this.formSuspensiones.value.desde !== null && this.formSuspensiones.value.hasta !== null) {
         this.suspeService.getByFecha(this.formSuspensiones.value.desde, this.formSuspensiones.value.hasta).subscribe({
            next: (datos) => {
               this.suspensiones = datos;
            },
            error: (e) => console.error(e)

...
export class HabilitacionesComponent implements OnInit {
   ...
   constructor(private fb: FormBuilder, private router: Router, private suspeService: SuspensionesService, private s_abonado: AbonadosService,
      private s_aboxsuspension: AboxsuspensionService, private s_documentos: DocumentosService, private coloresService: ColoresService) { }
   ...
   buscarxFecha() {
      if (
         ...
         this.suspeService.getListaHabilitacionesByFecha(this.f_buscarxFechas.value.desde, this.f_buscarxFechas.value.hasta).subscribe({
            next: (datos) => {
               this.suspensiones = datos;
            },
            error: (e) => console.error(e),

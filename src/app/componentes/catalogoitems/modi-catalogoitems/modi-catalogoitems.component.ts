...
export class ModiCatalogoitemsComponent implements OnInit {
  ...
  constructor(
    ...
    private produService: CatalogoitemService,
  ...
  listarModulos() {
    this.moduService.getListaModulos().subscribe({
      next: (resp) => {
        ...
        setTimeout(() => {
          let opcModulo = document.getElementById(
          ...
          if (opcModulo != null) {
            ...
            this.usoService.getByIdmodulo(this.idmodulo).subscribe({
              next: (resp) => {
                ...
                setTimeout(() => {
                  let opcUso = document.getElementById(
                    `idusoitems_` + this.idusoitems
                  ) as HTMLElement;
  ...
  listenerChanges() {
    ...
    selectuso.addEventListener('change', () => {
      ...
      this.idusoitems =
        this._usos[
          +selectuso.value.slice(0, selectuso.value.indexOf(':'))
        ].idusoitems;
    });
  }
  ...
  valNombre(control: AbstractControl) {
    return this.produService
      .getByNombre(3, control.value)
      .pipe(
        map((result) =>

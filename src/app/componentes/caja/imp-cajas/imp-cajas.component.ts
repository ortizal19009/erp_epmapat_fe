...
export class ImpCajasComponent implements OnInit {
   ...
   constructor(public authService: AutorizaService, public fb: FormBuilder,
      private router: Router, private cajService: CajaService, private facService: FacturaService,
      private rxfService: RubroxfacService, private _pdf: PdfService) { }
      ...
      switch (this.opcreporte) {
         case 1:  // Recaudacion diaria - Resumen
            try {
               ...
               try {
                  ...
                  try {
                     this._formacobro = await this.facService.totalFechaFormacobroAsync(fecha);
                     console.log('this._formacobro: ', this._formacobro)
                     if (this.swimprimir) this.imprimirResumen();
                     // else this.exportarFacturas();
                  } catch (error) {
                     console.error('Error al obtener los totales por Forma de cobro:', error);
                  }
                  // if (this.swimprimir) this.imprimirResumen();
                  // else this.exportarFacturas();
               } catch (error) {
         ...
         case 2:  // Recaudacion diaria - Planillas
            try {
               ...
               console.error('Error al obtener this._cobradas:', error);
            }
            break;
   ...
   retornar() { this.router.navigate(['/cajas']); }
   ...
   imprimirResumen() {
      ...
      //doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("RESUMEN RECAUDACIÓN DIARIA: " + this.formImprimir.value.fecha, m_izquierda, 16);
      this._pdf.header(`RESUMEN RECAUDACIÓN DIARIA: ${this.formImprimir.value.fecha, m_izquierda}`, doc);
      ...
      this._cobradas.forEach(() => {
         ...
         datos.push([this._cobradas[i][0], this._cobradas[i][1], totalRecaudado.toFixed(2)]);
         suma += totalRecaudado;
         i++;
      });
      kont = kont + i;
      this.total += suma;
      datos.push(['', 'SUBTOTAL', suma.toLocaleString("es-ES", { maximumFractionDigits: 2 })]);

      let suma1 = 0;
      i = 0; datos.push(['', 'PERÍODOS ANTERIORES']);
      this._rubrosanterior.forEach(() => {
         let totalRecaudado = Math.round(this._rubrosanterior[i][2] * 100) / 100;
         datos.push([this._rubrosanterior[i][0], this._rubrosanterior[i][1], totalRecaudado.toFixed(2)]);
         suma1 += totalRecaudado;
         i++;
      });
      kont = kont + i;
      this.total += suma1;
      datos.push(['', 'SUBTOTAL', suma1.toLocaleString("es-ES", { maximumFractionDigits: 2 })]);

      // const datos1: any = [];
      // let suma1: number = 0;
      // var i = 0;
      // this._cobradas.forEach(() => {
      //    let totalRecaudado = Math.round(this._cobradas[i][1] * 100) / 100;
      //    datos.push([this._cobradas[i][0], totalRecaudado]);
      //    suma += totalRecaudado;
      //    i++;
      // });
      // datos.push(['TOTAL', suma.toLocaleString("es-ES", { maximumFractionDigits: 2 })]);
      ...
      autoTable(doc, {
         ...
         didParseCell: function (data) {
            ...
            // if (columna > 0 && typeof data.cell.raw == 'number') { data.cell.text = [data.cell.raw.toLocaleString("es-ES", { maximumFractionDigits: 2 })]; }
            if (fila === datos.length - 1) { data.cell.styles.fontStyle = 'bold'; }
         }
      });
      ...
      this._formacobro.forEach(() => {
         ...
         formascobro.push([this._formacobro[i][0], totalRecaudado]);
         suma2 += totalRecaudado;
         i++;
      });
      formascobro.push(['TOTAL', suma2.toLocaleString("es-ES", { maximumFractionDigits: 2 })]);
      ...
      autoTable(doc, {
         ...
         didParseCell: function (data) {
            ...
            var columna = data.column.index;
            // if (columna > 0 && typeof data.cell.raw == 'number') { data.cell.text = [data.cell.raw.toLocaleString("es-ES", { maximumFractionDigits: 2 })]; }
            if (fila === formascobro.length - 1) { data.cell.styles.fontStyle = 'bold'; }
         }
      });
   ...
   imprimirFacturas() {
      ...
      let doc = new jsPDF('p', 'pt', 'a4');
      //doc.setFont("times", "bold"); doc.setFontSize(16); doc.text("EpmapaT", m_izquierda, 10);
      //doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("RECAUDACIÓN DIARIA - PLANILLAS: " + this.formImprimir.value.fecha, m_izquierda, 16);
      this._pdf.header("RECAUDACIÓN DIARIA - PLANILLAS: " + this.formImprimir.value.fecha, doc);
      const datos: any = [];
      let suma: number = 0;
      var i = 0;
      this._cobradas.forEach(() => {
         let totalPorFormaCobro = Math.round(this._cobradas[i][1] * 100) / 100;
         datos.push([this._cobradas[i][0].idfactura, this._cobradas[i][0].feccrea, this._cobradas[i][0].formapago, totalPorFormaCobro.toFixed(2)]);
         // suma += Math.round(this._cobradas[i].totaltarifa * 100) / 100;
         suma += totalPorFormaCobro;
         i++;
      });
      this.sumtotaltarifa = suma;
      datos.push(['', 'TOTAL', '', this.sumtotaltarifa.toLocaleString("es-ES", { maximumFractionDigits: 2 })]);
      ...
      autoTable(doc, {
         head: [['Nro', 'Fecha', 'Forma Cobro', 'Valor']],
         theme: 'grid',
         headStyles: { fillColor: [68, 103, 114], fontStyle: 'bold', halign: 'center' },
         ...
         margin: { left: m_izquierda - 1, top: 18, right: 87, bottom: 13 },
         // body: datos,
         body: datos,
         // body: jsonCobradas,
   ...
   imprimirCajas() {
      ...
      doc.setFont("times", "bold"); doc.setFontSize(16); doc.text("EpmapaT", m_izquierda, 10);
      doc.setFont("times", "bold"); doc.setFontSize(12); doc.text("LISTA DE CAJAS", m_izquierda, 16);
      ...
      var i = 0;
      this._cajas.forEach(() => {
         datos.push([this._cajas[i].idptoemision_ptoemision.establecimiento, this._cajas[i].codigo, this._cajas[i].descripcion,]);
         i++;
      });
   ...
   exportarCajas() {
      ...
      this._cajas.forEach((caja: any) => {
         const row = [caja.codigo, caja.idptoemision_ptoemision.establecimiento, caja.descripcion];
      ...
      // for (let i = 4; i <= this._cajas.length + 2; i++) {
   ...
   exportarFacturas() {
      ...
      const workbook = new ExcelJS.Workbook();
      const worksheet = workbook.addWorksheet(this.nombrearchivo);
      let titulo = 'Recaudacion diaria - Planillas ' + this.formImprimir.value.fecha
      ...
      worksheet.addRow([]);
      const cabecera = ['Nro', 'Fecha', 'Nro.Factura', 'Cliente', 'Sección', 'Forma cobro', 'Valor'];
      const headerRowCell = worksheet.addRow(cabecera);
      headerRowCell.eachCell(cell => {
         cell.fill = {
            type: 'pattern',
      ...
      this._cobradas.forEach((factura: any) => {
         let total = factura[1];
         const row = [factura[0].idfactura, factura[0].feccrea, factura[0].nrofactura, factura[0].idcliente.nombre, factura[0].idmodulo.descripcion,
         factura[0].formapago, total];
      ...
      worksheet.getCell('B' + (this._cobradas.length + 4).toString()).font = { bold: true }

      let celdaG = worksheet.getCell('G' + (this._cobradas.length + 4).toString());
      celdaG.numFmt = '#,##0.00';
      celdaG.font = { bold: true }
      celdaG.value = { formula: 'SUM(G4:' + 'G' + (this._cobradas.length + 3).toString() + ')', result: 0, sharedFormula: undefined, date1904: false };

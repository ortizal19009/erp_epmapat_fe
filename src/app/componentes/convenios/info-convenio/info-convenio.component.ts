...
export class InfoConvenioComponent implements OnInit {
   ...
   constructor(private convService: ConvenioService, private fxconvService: FacxconvenioService,
      private cuotaService: CuotasService, private facService: FacturaService,
      private rxfService: RubroxfacService, private router: Router) { }
   ...
   datosConvenio() {
      ...
      this.convService.getById(+idconvenio!).subscribe(datos => {
   ...
   cuotasxConvenio(idconvenio: number) {
      this.cuotaService.getByIdconvenio(idconvenio).subscribe(datos => {
         this._cuotas = datos;
...
//         if (this._cuotas[0].idfactura.nrofactura.length > 3) { this.elimdisabled = false; }
   ...
   nombreCliente(idfactura: number) {
      this.facService.getById(idfactura).subscribe(datos => {
   ...
   facxConvenio() {
      let idconvenio = sessionStorage.getItem('idconvenioToInfo');
      this.fxconvService.getFacByConvenio(+idconvenio!).subscribe(datos => {
   ...
   totalFacturas() {
      ...
      if (Number(this.convenio.totalconvenio.toFixed(2)) - this.total2 != 0) { this.dif2 = true; }
   }
   ...
   confirmaEliminarConvenio() {
      ...
      if (idc != null) {
         this.convService.deleteConvenio(+idc!).subscribe(datos => {
         }, error => console.log(error));
      }
   }

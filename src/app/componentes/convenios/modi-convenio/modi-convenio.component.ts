...
export class ModiConvenioComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.idconvenio === 0) {
      ...
      this.getDatoConvenio(this.idconvenio);
          this.datosConvenio();
  ...
  anular() {}
  eliminar() {}
  async getDatoConvenio(idconvenio: number) {
    let convenio: any = await this.convService
      .getPendienteByConvenio(idconvenio)
      .toPromise();
    console.log(convenio);
    convenio.length > 0 ? (this.swdeanul = true) : (this.swdeanul = false);
  }
}

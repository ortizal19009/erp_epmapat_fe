...
export class ListarCertificacionesComponent implements OnInit {
   ...
   constructor(private coloresService: ColoresService, public certiService: CertificacionesService, public router: Router, private fb: FormBuilder) { }
   ...
   ngOnInit(): void {
      ...
      this.certiService.ultima().subscribe({
         next: datos => {
            let hasta: string = '';
            if (sessionStorage.getItem("hastaCertificaciones") != null) hasta = sessionStorage.getItem("hastaCertificaciones")!;
            else hasta = datos.numero.toString();

            let desde: string = '';
            if (sessionStorage.getItem("desdeCertificaciones") != null) desde = sessionStorage.getItem("desdeCertificaciones")!
            ...
            let cliente: string = '';
            if (sessionStorage.getItem("clienteCertificaciones") != null) cliente = sessionStorage.getItem("clienteCertificaciones")!
   ...
   async buscaColor() {
      try {
         const datos = await this.coloresService.setcolor(1, 'certificaciones');
   ...
   public buscar() {
      ...
      sessionStorage.setItem("clienteCertificaciones", this.formBuscar.value.cliente);
      ...
      if (cliente != '' && cliente != null) {
         ...
         sessionStorage.setItem("desdeCertificaciones", this.formBuscar.value.desde);
         sessionStorage.setItem("hastaCertificaciones", this.formBuscar.value.hasta);
         ...
         this.certiService.getDesdeHasta(desde, hasta).subscribe({
            ...
            error: err => console.error('Al recuperar las Certificaciones: ', err.error)
         });
      }
   }
   ...
   modiCertificacion(certificaciones: Certificaciones) {
      sessionStorage.setItem("idcertificacionToModi", certificaciones.idcertificacion.toString());
   ...
   aprobarEliminacionCertificacion() {
      ...
      this.certiService.deleteCertificaciones(+idc!).subscribe(datos => {

...
export class ModiDocumentoComponent implements OnInit {
   ...
   constructor(public fb: FormBuilder, public docuService: DocumentosService,
      private tab4Service: Tabla4Service, private router: Router) { }
   ...
   ngOnInit(): void {
      ...
      this.formDocumento = this.fb.group({
         nomdoc: [null, [Validators.required, Validators.minLength(3)], this.valNomdoc.bind(this)],
   ...
   onSubmit() {
      this.docuService.updateDocumento(this.iddocumento, this.formDocumento.value).subscribe({
         next: resp => this.retornar(),
         error: err => console.log(err.error)
      });
   }

   retornar() { this.router.navigate(['/info-documento']); }

   compararTabla4(o1: Tabla4, o2: Tabla4): boolean {
      if (o1 === undefined && o2 === undefined) { return true; }
      else {
         return o1 === null || o2 === null || o1 === undefined || o2 === undefined ? false : o1.idtabla4 == o2.idtabla4;
      }
   }
   ...
   valNomdoc(control: AbstractControl) {
      return this.docuService.getByNomdoc(control.value)
         .pipe(
            map(result => result.length == 1 && control.value != this.antnomdoc ? { existe: true } : null)
         );
   }

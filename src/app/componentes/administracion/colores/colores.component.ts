...
export class ColoresComponent implements OnInit {
  ...
  constructor(
    ...
    private venServicio: VentanasService,
  ...
  ngOnInit(): void {
    ...
    if (!this.nomVentana || this.nomVentana == '/inicio') {
      ...
      sessionStorage.setItem(
        '/cajas',
        JSON.stringify(['rgb(57, 95, 95)', 'rgb(207, 221, 210)'])
      );
  ...
  async actualizar() {
    const rgb0 =
      'rgb(' +
      this.colorForm.get('red0')?.value +
      ',' +
      this.colorForm.get('green0')?.value +
      ',' +
      this.colorForm.get('blue0')?.value +
      ')';
    const rgb1 =
      'rgb(' +
      this.colorForm.get('red1')?.value +
      ',' +
      this.colorForm.get('green1')?.value +
      ',' +
      this.colorForm.get('blue1')?.value +
      ')';
    ...
    ventana.permissions = 1;
    // Busca la ventana por usuario y nombre para luego actualizarla
    let ventanas = await this.venServicio.getByIdusuarioyNombre(
  ...
  regresar() {
    this.router.navigate([this.nomVentana]);
  }
}

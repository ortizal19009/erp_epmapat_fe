...
export class ModiTabla4Component implements OnInit {
   ...
   constructor(public fb: FormBuilder, public tab4Service: Tabla4Service, private router: Router) { }
   ...
   ngOnInit(): void {
      ...
      this.tab4Form = this.fb.group({
         ...
         nomcomprobante: [null, [Validators.required, Validators.minLength(3)], this.valNomcomprobante.bind(this)],
         usucrea: 1,
         feccrea: '',
         usumodi: 1,
         fecmodi: ''
   ...
   onSubmit() {
      this.tab4Service.updateTabla4(this.idtabla4, this.tab4Form.value).subscribe({
         next: resp => this.retornar(),
         error: err => console.log(err.error)
      });
   }

   retornar() { this.router.navigate(['/info-tabla4']); }

   valTipocomprobante(control: AbstractControl) {
      return this.tab4Service.getByTipocomprobante(control.value)
         .pipe(
            map(result => result.length == 1 && control.value != this.antTipocomprobante ? { existe: true } : null)
         );
   }
   ...
   valNomcomprobante(control: AbstractControl) {
      return this.tab4Service.getByNomcomprobante(control.value)
         .pipe(
            map(result => result.length == 1 && control.value != this.antNomcomprobante ? { existe: true } : null)
         );
   }
}

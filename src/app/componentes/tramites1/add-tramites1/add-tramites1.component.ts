...
export class AddTramites1Component implements OnInit {
  ...
  ngOnInit(): void {
    ...
    modulos.idmodulo = 50;
    setTimeout(() => {
      let o_modulos = document.getElementById(
        'bs-' + modulos.idmodulo
  ...
  guardarTramite() {
    ...
    this.f_tramite.value.idabonado_abonados =
      this.f_tramite.value.idabonado_abonados[0];
    this.s_tramites1.saveTramites1(this.f_tramite.value).subscribe(
      (datos) => {
        this.generarFactura();
        setTimeout(() => {
  ...
  obtenerUltimoTramite() {
    this.s_tramites1.getListaTramites1().subscribe(
      (datos) => {
  ...
  obtenerUltimaFactura() {
    this.s_facturas.getListaFacturas().subscribe(
      (datos) => {
  ...
  listarModulos() {
    this.s_modulos.getTwoModulos().subscribe(
      (datos) => {
  ...
  listarServicios() {
    ...
    this.s_servicios.getByIdModulo(50).subscribe(
      (datos) => {
    ...
    s_modulos.addEventListener('change', () => {
      ...
      if (+v_modulo! === 50) {
        ...
        this.arr_rubro = [];
        this.s_servicios.getByIdModulo(+v_modulo!).subscribe(
          (datos) => {
            this.v_servicios = datos;
          },
          (error) => console.error(error)
        ...
        this.arr_rubro = [];
        this.s_rubro.getByIdmodulo(+v_modulo!).subscribe(
          (datos) => {
            this.v_rubros = datos;
          },
          (error) => console.error(error)
  ...
  obtenerValoresClientes(cliente: Clientes) {
    ...
    this.s_abonados
      .getListaByidentIficacionCliente(c_cliente.toString())
      .subscribe(
        (datos) => {
  ...
  obtenerServicios(event: any) {
    if (event.target.checked) {
      ...
      this.s_servicios.getListaById(num).subscribe(
        (datos) => {
      ...
      let result = arr.find(
        (result: { idservicio: any }) => result.idservicio == num
      );
  ...
  selRubros(event: any) {
    ...
    let result = rubros.find(
      (result: { idrubro: any }) => result.idrubro == num
    );
  ...
  obtenerRubros(event: any) {
    if (event.target.checked) {
      let num: number = event.target.value;
      this.s_rubro.getRubroById(num).subscribe(
        (datos) => {
          this.arr_rubro.push(datos);
          this.sumaTotal();
        },

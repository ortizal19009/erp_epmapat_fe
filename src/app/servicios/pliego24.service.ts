...
export class Pliego24Service {
  ...
  getBloque(idcategoria: number, m3: number): Observable<Pliego24[]> {
    ...
    return this.http.get<Pliego24[]>(
      `${baseUrl}/bloque?idcategoria=${idcategoria}&m3=${m3}`
    );
  }
  //Una Tarifa del Pliego async
  async getBloqueAsync(idcategoria: number, m3: number): Promise<Pliego24[]> {
    const response = await firstValueFrom(
      this.http.get<Pliego24[]>(
        `${baseUrl}/bloque?idcategoria=${idcategoria}&m3=${m3}`
      )
    );
    return response;
